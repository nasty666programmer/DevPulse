...
> **Манифесты и деплой переехали.** `infra/k8s` и сам механизм SSH-деплоя (`.github/workflows/deploy.yml`
> в прежнем виде) теперь живут в отдельном приватном репозитории
> [`nasty666programmer/pulsedev-infra`](https://github.com/nasty666programmer/pulsedev-infra) —
> вынесли, чтобы топология кластера и адрес сервера не лежали в публичном репо (этот репо
> публичный ради бесплатного CI). Этот файл описывает изначальные проектные решения и
> остаётся здесь как обоснование архитектуры; пути вида `infra/k8s/...` ниже читай как
> `k8s/...` в `pulsedev-infra`. Все команды `kubectl`/`kustomize` теперь выполняются из
> чекаута `pulsedev-infra`, а не отсюда.
